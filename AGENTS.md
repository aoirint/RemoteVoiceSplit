...
- `RemoteVoiceSplit/Plugin.cs` is the BepInEx entry point. Keep startup
  limited to logger setup and integration lifecycle wiring.
- `RemoteVoiceSplit/Core/` owns framework-independent buffering, mixing,
  protocol, capture, and process-ancestry behavior.
- `RemoteVoiceSplit/Interop/Game/` owns reflection-based game integration.
  Keep base-game assemblies out of build dependencies.
- `RemoteVoiceSplit/Interop/ProcessAudio/` owns shell launch, process-tree, and
  named-pipe client boundaries.
- `RemoteVoiceSplit.AudioHost/Interop/WindowsAudio/` owns Core Audio and WASAPI
  resources in the OBS-capturable helper process.
- `RemoteVoiceSplit.Tests/` is the deterministic console test and
  package-contract harness.
- `RemoteVoiceSplit.slnx` is the solution entry point.
...
Use `.agents/skills/domain-architecture-docs-workflow/` when creating,
