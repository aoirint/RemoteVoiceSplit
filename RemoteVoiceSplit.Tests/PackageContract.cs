...
internal static class PackageContract
{
    ...
    public static void Validate(string archivePath, string expectedProjectVersion, string expectedArtifactVersion)
    {
        ...
        string expectedArchiveName = $"RemoteVoiceSplit-v{expectedArtifactVersion}.zip";
        ...
        {
            throw new InvalidDataException("Archive must contain exactly eight files.");
        }
    ...
    private static void ValidateManifest(string manifest, string expectedVersion)
    {
        ...
        if (!string.Equals(parsed.Name, "RemoteVoiceSplit", StringComparison.Ordinal))
        {
    ...
    private static void ValidatePluginAssembly(byte[] assemblyBytes, string expectedVersion)
    {
        try
        {
            ...
            if (!string.Equals(module.Assembly.Name.Name, "RemoteVoiceSplit", StringComparison.Ordinal))
            {
    ...
    private static void ValidateAudioHostAssembly(byte[] assemblyBytes, string expectedVersion)
    {
        try
        {
            using var stream = new MemoryStream(assemblyBytes, writable: false);
            using ModuleDefinition module = ModuleDefinition.ReadModule(stream);
            if (!string.Equals(module.Assembly.Name.Name, "RemoteVoiceSplit.AudioHost", StringComparison.Ordinal))
            {
        ...
        {
            throw new InvalidDataException("Packaged audio host EXE is not a valid managed assembly.", exception);
        }
    }

    private static void ValidateAudioHostConfiguration(string configuration)
    {
        try
        {
            XDocument document = XDocument.Parse(configuration, LoadOptions.None);
            XElement? runtime = document.Root?.Element("startup")?.Element("supportedRuntime");
            if (!string.Equals((string?)runtime?.Attribute("version"), "v4.0", StringComparison.Ordinal)
                || !string.Equals(
                    (string?)runtime?.Attribute("sku"),
                    ".NETFramework,Version=v4.8",
                    StringComparison.Ordinal))
            {
                throw new InvalidDataException(
                    "Packaged audio host configuration must select .NET Framework 4.8.");
            }
        }
        catch (System.Xml.XmlException exception)
        {
            throw new InvalidDataException(
                "Packaged audio host configuration is not valid XML.",
                exception);
        }
    }
...
internal static class PackageContractTests
{
        ...
        string? archivePath)
    {
        ...
        try
        {
            ...
            AssertRejected("corrupt-dll", Replace(validSources, "RemoteVoiceSplit.dll", new("RemoteVoiceSplit.dll", new byte[] { 1, 2, 3 })), tempRoot, expectedVersion, "not a valid managed assembly");
            AssertRejected("corrupt-host", Replace(validSources, "RemoteVoiceSplit.AudioHost.exe", new("RemoteVoiceSplit.AudioHost.exe", new byte[] { 1, 2, 3 })), tempRoot, expectedVersion, "not a valid managed assembly");
            AssertRejected(
                "corrupt-host-config",
                Replace(
                    validSources,
            ...
            AssertRejected(
                "manifest-version",
                Replace(validSources, "manifest.json", new("manifest.json", ManifestBytes("9.9.9", "RemoteVoiceSplit"))),
                tempRoot,
                expectedVersion,
        ...
        string repositoryRoot) => new[]
    {
        ...
        PackageFixtureBuilder.ArchiveSource.FromFile(
            "RemoteVoiceSplit.AudioHost.exe.config",
    ...
    private static void AssertRejectedArchiveSize(string tempRoot, string expectedVersion)
    {
        ...
        string archivePath = Path.Combine(directory, $"RemoteVoiceSplit-v{expectedVersion}.zip");
        ...
        string expectedVersion)
    {
        ...
        string archivePath = Path.Combine(fixtureDirectory, $"RemoteVoiceSplit-v{expectedVersion}.zip");
    ...
    private static byte[] MisleadingManifestBytes(string expectedVersion) =>
        JsonSerializer.SerializeToUtf8Bytes(new
        {
            description = $"name RemoteVoiceSplit version_number {expectedVersion} BepInEx-BepInExPack-5.4.2305",
            name = "WrongName",
    ...
    {
        PackageFixtureBuilder.ArchiveSource assembly = sources.Single(source => source.Name == "RemoteVoiceSplit.dll");
