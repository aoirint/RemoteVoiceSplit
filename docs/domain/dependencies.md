...
| `Microsoft.NETFramework.ReferenceAssemblies.net48` | 1.0.3 | Portable .NET Framework 4.8 compile references for the host | Build-only |

No game DLL or exported asset is a build dependency or distributable output.
The package contains only the mod's `RemoteVoiceSplit.dll`,
`RemoteVoiceSplit.AudioHost.exe`, its generated .NET Framework configuration,
package metadata, documentation, icon, and license.

## Adoption review

The official BepInEx registration feed and nuget.org metadata reported these
publication dates during the 2026-07-22 and 2026-07-23 reviews:
...
- `Microsoft.NETFramework.ReferenceAssemblies.net48` 1.0.3: 2022-08-19,
  Microsoft package with no dependencies.
...
the host's .NET Framework and Windows Core Audio calls, and build-time
diagnostics or metadata generation. The reference-assemblies package is not
